...
impl Error {
    ...
    #[async_move]
    pub fn typ(n: Normal, t: String) -> Result<Self> {
        let s = await!(n.to_string())?;
        Ok(Self::typ_raw(&format!("{} is not a {}", s, t)))
    }

    #[async_move]
    pub fn not_boolean(n: Normal) -> Result<Self> {
    ...
    #[async_move]
    pub fn not_collection(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "collection".to_string()))
    }

    #[async_move]
    pub fn not_dictionary(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "dictionary".to_string()))
    }

    #[async_move]
    pub fn not_function(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "function".to_string()))
    }

    #[async_move]
    pub fn not_list(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "list".to_string()))
    }

    #[async_move]
    pub fn not_nil(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "nil".to_string()))
    }

    #[async_move]
    pub fn not_number(n: Normal) -> Result<Self> {
        await!(Self::typ(n, "number".to_string()))
    }

    #[async_move]
    pub fn not_string(n: Normal) -> Result<Self> {
