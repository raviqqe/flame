...
impl<'a> Interpreter<'a> {
    ...
    pub fn interpret_function(&mut self) -> Value {
        let f = self.variables[self.code[self.index] as usize].clone();
        self.index += 1;
        f
    }

    pub fn interpret_arguments(&mut self) -> Arguments {
        let ps = self.interpret_positional_arguments();
        let ks = self.interpret_keyword_arguments();
        Arguments::new(&ps, &ks)
    }

    pub fn interpret_positional_arguments(&mut self) -> Vec<Expansion<Value>> {
        unimplemented!()
    }

    pub fn interpret_keyword_arguments(&mut self) -> Vec<Expansion<KeywordArgument>> {
        unimplemented!()
    }
}
